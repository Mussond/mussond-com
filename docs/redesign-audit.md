> **Superseded (7.8.3, phase 7 closeout).** This is a baseline snapshot from an earlier, abandoned redesign attempt (`redesign/sprint-1-foundation`) and no longer reflects the shipped system: body font is Source Sans 3 (not Public Sans), the contrast hook is `[data-contrast="low"|"high"|"system"]` with a separate `[data-flourish="full"|"reduced"|"raw"]` axis (there is no `"decorative"` contrast value — it was retired and replaced by the flourish axis, see [tokens.css](../src/styles/tokens.css)), and the theme/no-flash mechanics live in `src/layouts/Layout.astro`. Kept for history only; treat nothing below as current. The screenshots in `docs/redesign-audit/screenshots/` have been refreshed to the current 12-state model (2 themes × 2 contrasts × 3 flourishes) as part of the 7.8 closeout sweep — the doc text describing them still refers to the old six-state model and is stale alongside the rest of this file.
